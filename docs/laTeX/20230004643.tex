...
\hline
RF02.1 & Cadastrar usuário paciente & Paciente & O usuário deverá preencher um formulário com campos obrigatórios, como nome completo, CPF, e-mail, data de nascimento, telefone, endereço e senha. Haverá um identificador do usuário. O usuário também deverá informar um código do profissional para vinculá-los.\\
\hline
RF02.2 & Cadastrar usuário prescritor & Prescritor & O usuário deverá preencher um formulário com campos obrigatórios, como nome completo, CPF, e-mail, data de nascimento, telefone, endereço, senha, profissão e o número do registro profisisonal (crm, coren, crbm, crp...). Haverá um identificador do usuário. O profissional possuirá um código único que será utilizado por seus pacientes no momento do cadastro para estabelecer vínculo.\\
\hline
...
\hline
RF04 & Consulta Clínica & Prescritor & Durante a consulta o prescritor deverá registrar em uma campo de consulta clícica as observações e dados clínicos relevantes. Em outro campo, o prescritor também poderá atualizar ou adicionar diagnósticos. Haverá também o registro de condutas e definição de novos tratamentos para o plano terapeutico. Caso haja necessidade, poderá emitir prescrições (RF5). A cada atendimento, também será possível registrar a evolução clínica do paciente, relatando melhoras, pioras ou manutenção do quadro, bem como quaisquer recomendações adicionais. Todos os registros inseridos na ficha clínica serão vinculados ao profissional responsável e paciente, devidamente datados. Deve haver um campo de modalidade de consulta, presencial ou remota, bem como um registro do status da consulta (em andamento, concluída...). \\
\hline
\end{tabularx}
\end{table}
...
\textbf{ID} & \textbf{Requisito} & \textbf{Usuário} & \textbf{Descrição}\\
\hline
RF05 & Prescrição & Prescritor & A prescrição será feita de forma digital, onde o prescritor poderá definir a descrição do produto, a marca, a concentração do óleo, a posologia, o espectro e quaisquer instruções específicas relacionadas ao tratamento. O sistema também permitirá a revisão e modificação de prescrições conforme necessário. Deve haver identificador único.\\
\hline
...
\hline
RF08 & Tela de Escalas Clínicas & Paciente & O paciente terá acesso a uma tela central de escalas clínicas, onde serão listadas todas as escalas de avaliação pendentes ou já preenchidas. As escalas são organizadas em duas seções distintas, uma de pendentes e outra de histórico. Na seção de pendentes, cada avaliação é apresentada em um card individual com um indicador visual de status, mostrando o nome da escala e um botão de ação para preencher que redirecionará para a tela daquela escala. A seção de histórico exibirá os itens em formato de tabela com colunas para nome da escala, data de conclusão, resultado e ações. As ações disponíveis para cada item do histórico são os botões de ver respostas e de ver progresso. Ver resposta redirecionará para a tela daquela escala preenchida. Ver progresso redirecionará para a tela de progresso\\
\hline
RF09 & Tela de Escalas Clínicas & Prescritor & O prescritor poderá selecionar escalas padronizadas existentes para disponibilizar aos pacientes.\\
\hline
\end{tabularx}
\end{table}
...
\hline
RF19 & Tela de Anamnese & Paciente & O usuário irá preencher os seguintes campos: profissão, motivo principal da consulta, diagnóstico(s) prévio(s), tratamentos anteriores, medicações em uso, tipo de dieta, hábito de fumar, uso de álcool, peso, altura, uso de substâncias recreativas, prática de exercícios físicos, qualidade do sono, presença de ansiedade, presença de dor, histórico familiar de doenças, reação adversas, condições genéticas, expectativas com o tratamento, formas de monitoramento e observações gerais (Figura 3). Deve haver um identificador único para as anamneses. Deve haver registro da data de preenchimento.\\
\hline
\end{tabularx}
\end{table}
...
\rowcolor{gray!20}
\textbf{ID} & \textbf{Requisito} & \textbf{Usuário} & \textbf{Descrição}\\
RF20 & Tela de Ficha de Acompanhamento & Paciente & O usuário irá a quantidade de gotas utilizada pela manhã e à tarde, e avaliar os seguintes parâmetros em escala de 0 a 10: dor (0 = sem dor, 10 = dor intensa), sono (0 = muito ruim, 10 = excelente), humor (0 = deprimido, 10 = muito positivo), tremor (0 = ausente, 10 = grave), ansiedade (0 = muito ansioso, 10 = tranquilo), disposição/energia (0 = sem energia, 10 = muito energético), função intestinal (0 = irregular, 10 = normal), apetite (0 = sem apetite, 10 = saudável), concentração (0 = muito baixa, 10 = excelente), interação social (0 = isolado, 10 = muito social), rigidez/espasticidade (0 = nenhuma, 10 = intensa), redução no uso de maconha fumada/outra substância (0 = nenhuma, 10 = completa), náusea e vômito (0 = frequente, 10 = ausente), desempenho em atividades esportivas (0 = baixo, 10 = excelente) e presença de doença dermatológica (0 a 10) (Figura 4). Haverá também um campo de comentário opcional. Deve haver registro da data de preenchimentoe identificador único. \\
\hline
...
\textbf{ID} & \textbf{Requisito} & \textbf{Usuário} & \textbf{Descrição}\\
\hline
RF27 & Tela de progresso & Paciente & A tela de progresso do paciente deve exibir a evolução de seus dados clínicos através de um seletor de período no topo da tela, que permite ao usuário definir o intervalo de tempo para a análise (últimos 15 dias, 30 dias, 60 dias, 90 dias...). Também será possível selecionar a escala desejada para visualização e filtrar o atributo que deseja visualizar o progresso. A visualização principal é composta por um gráfico de evolução do atributo selecionado e monitorado da escala selecionada (eixo Y) ao longo das datas de preenchimento (eixo X).\\
\hline
RF28 & Tela de progresso & Pescritor & A tela de progresso do prescritor deve exibir a evolução dos dados clínicos do paciente selecionado através de um seletor de período no topo da tela, que permite ao prescritor definir o intervalo de tempo para a análise (todo o tempo, últimos 15 dias, 30 dias...). Também será possível selecionar a escala desejada para visualização e filtrar o atributo que deseja visualizar o progresso. A visualização principal é composta por um gráfico de evolução do atributo selecionado e monitorado da escala selecionada (eixo Y) ao longo das datas de preenchimento (eixo X).\\
\hline
\end{tabularx}
\end{table}
